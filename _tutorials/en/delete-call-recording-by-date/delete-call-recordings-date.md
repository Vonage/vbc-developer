...
2. Loop through the call recordings from the `get_company_call_recordings()` function you wrote and get the recording's `call_id`. You will pass in `call_id` into your `delete_call_recording()` function to delete the recording.
